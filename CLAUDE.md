...
- 絞り込みは `cargo test viewport` / `cargo test ui::` のようにモジュール名で指定する。失敗時の再実行ヒントには `--bin gdf` が付く
- 初回ビルドは libgit2 のコンパイルを含み 40 秒前後かかる
- `main` は rustfmt 済みではない（別 Issue で対応予定）。`cargo fmt` をツリー全体にかけない
...
- syntect の `HighlightLines` は状態を持つ。diff では旧版側と新版側で別々に持ち、ハンク境界でリセットする（`src/highlight.rs`）
